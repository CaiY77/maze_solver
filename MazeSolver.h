...
private:
    ...
    char** solution_ = nullptr;   //a 2-d character array containing maze characters as in maze_ and path to exit marked with '>' characters
...
/*
first thing maze solver has to do is to read the maze solver. the first 2 numbers are the number of rows and coloums in the maze. you need to put it into a 2d array
build a copy of the array and replace path with ">" solving the maze. Back tracking is replaced with "@"
you can use a std stack. vector for a stack.

solver will always explore south first then east.
